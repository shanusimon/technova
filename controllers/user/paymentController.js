...
    res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ message: "Unable to create order", error });
  }
};
...
      if (deletedOrder) {
        return res.status(StatusCodes.OK).json({ message: "Order cancelled and deleted" });
      } else {
        return res.status(StatusCodes.NOT_FOUND).json({ message: "Order not found" });
      }
    }
...
      );
      return res.status(StatusCodes.BAD_REQUEST).json({ message: "Invalid payment signature" });
    }
  } catch (error) {
    console.error("Error updating order:", error);
    await redis.del(lockKey);
    res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ message: "Order update failed", error });
  }
};
