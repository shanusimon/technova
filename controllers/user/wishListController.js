...
const addProductToWishlist = async (req, res) => {
    try {
        ...
        if (wishData) {
            const productExists = wishData.products.some(item => item.productId.toString() === productId);
            if (!productExists) {
            ...
            return res.status(201).json({ message: 'New wishlist created and product added' });
        }
    } catch (error) {
        console.error(error);
...
const removeFromWishlist = async (req, res) => {
    try {
        ...
        if (result) {
            ...
            res.status(404).json({ message: "Wishlist not found or unauthorized access" });
        }
    } catch (error) {
